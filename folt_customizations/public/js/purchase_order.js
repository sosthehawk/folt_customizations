...
//
// "Who is this waiting for?" used to be answered here too, in the form headline. It is now
// answered generically for every workflow doctype by folt_guide.js, off the same
// workflow.get_pending_approvers call -- so the version here has gone rather than being left to
// say the same thing a second time in a second place on the same form. The reasoning it carried
// is worth keeping and now lives in workflow_shape._movers: every non-final state answers the
// question, so nothing is keyed on a particular state's name.
